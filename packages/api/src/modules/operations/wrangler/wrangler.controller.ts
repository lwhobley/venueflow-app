...
class ExecuteWranglerActionDto {
  @IsString()
  @IsIn(['REASSIGN_RESERVATION', 'NOTIFY_STAFF', 'CREATE_FOLLOW_UP'])
  type!: 'REASSIGN_RESERVATION' | 'NOTIFY_STAFF' | 'CREATE_FOLLOW_UP';
  @IsOptional() @IsString() reservationId?: string;
  @IsOptional() @IsString() tableId?: string;
  @IsOptional() @IsString() priorityId?: string;
}
...
export class WranglerController {
  ...
  async executeAction(@VenueScope() scope: Scope, @Body() body: ExecuteWranglerActionDto) {
    ...
    if (body.type === 'CREATE_FOLLOW_UP') {
      ...
      const snapshot = await this.wrangler.getSnapshot(scope.venueId, venue.timezone);
      const priority = snapshot.priorities.find((item) => item.id === body.priorityId);
      if (!priority || priority.kind === 'steady') throw new BadRequestException('Wrangler priority is no longer active');
      const targetDate = zonedIsoDate(venue.timezone, Date.now());
      const title = `Wrangler: ${priority.title}`;
      const existing = await this.prisma.managerGoal.findFirst({
        where: { venueId: scope.venueId, title, targetDate, status: 'open' },
        select: { id: true, title: true },
      });
      if (existing) return { ok: true, type: body.type, followUpId: existing.id, title: existing.title, existing: true };

      const created = await this.prisma.managerGoal.create({
        data: {
          venueId: scope.venueId,
          title,
          details: `${priority.reason} Recommended move: ${priority.cta}.`,
          period: 'day',
          targetDate,
          status: 'open',
      ...
      await this.prisma.auditLog.create({
        data: {
          ...
          entityType: 'manager_goal',
          entityId: created.id,
          action: 'wrangler_follow_up_created',
          summary: `Created follow-up from Wrangler priority: ${priority.title}`,
        },
      });
