...
const externalReservationSource = v.union(
  reservationSource,
  v.literal("sevenrooms"),
  v.literal("tock"),
  v.literal("google"),
);
const privateEventStatus = v.union(
  v.literal("inquiry"),
  v.literal("proposal"),
...
const assignmentHoldType = v.union(
  v.literal("reserved"),
  v.literal("held"),
  v.literal("seated"),
);
const posProvider = v.union(
  v.literal("toast"),
  v.literal("square"),
  v.literal("clover"),
  v.literal("generic"),
);
const posConnectionStatus = v.union(
  v.literal("connected"),
  v.literal("paused"),
  v.literal("error"),
);
const posCheckStatus = v.union(
  v.literal("open"),
  v.literal("paid"),
  v.literal("void"),
);
const reservationProvider = v.union(
  v.literal("opentable"),
  v.literal("resy"),
  v.literal("sevenrooms"),
  v.literal("tock"),
...
const barStockCategory = v.union(
  v.literal("spirit"),
  v.literal("wine"),
  v.literal("beer"),
  v.literal("mixer"),
...
const barStockMovementType = v.union(
  v.literal("count"),
  v.literal("received"),
  v.literal("waste"),
  v.literal("comp"),
...
const managerGoalStatus = v.union(
  v.literal("open"),
  v.literal("done"),
  v.literal("cancelled"),
);
...
export default defineSchema({
  ...
  venues: defineTable({
    ...
    subscriptionPlatform: v.optional(
      v.union(v.literal("stripe"), v.literal("apple"), v.null()),
    ),
  ...
  notificationEvents: defineTable({
    ...
    .index("by_venue_and_createdAt", ["venueId", "createdAt"])
  ...
  scheduleEmailEvents: defineTable({
    ...
    .index("by_venue_and_sentAt", ["venueId", "sentAt"])
    .index("by_profile_and_sentAt", ["profileId", "sentAt"]),
  ...
  timeEntries: defineTable({
    ...
    .index("by_profileId_and_isOpen", ["profileId", "isOpen"])
    .index("by_venueId", ["venueId"])
    .index("by_venue_clockInAt", ["venueId", "clockInAt"])
  ...
  posConnections: defineTable({
    ...
    .index("by_venue", ["venueId"])
    .index("by_venue_and_provider", ["venueId", "provider"]),
  posChecks: defineTable({
    ...
    .index("by_provider_external", ["provider", "externalCheckId"])
    .index("by_venue_provider_external", [
      "venueId",
      "provider",
      "externalCheckId",
    ])
  ...
  posLaborPunches: defineTable({
    ...
    .index("by_venue_employee_businessDate_and_clockInAt", [
      "venueId",
      "externalEmployeeId",
      "businessDate",
      "clockInAt",
  ...
  reservations: defineTable({
    ...
    .index("by_venue_time", ["venueId", "reservationTime"])
    .index("by_venue_status", ["venueId", "status"])
    .index("by_guest", ["guestId"])
    .index("by_external_id", ["externalId"])
    .index("by_venue_external_id", ["venueId", "externalId"]),
  ...
  reservationSyncEvents: defineTable({
    ...
    .index("by_provider_external_id", ["provider", "externalEventId"])
    .index("by_venue_provider_external_id", [
      "venueId",
      "provider",
      "externalEventId",
    ])
    .index("by_venue_processedAt", ["venueId", "processedAt"]),
  barInventoryItems: defineTable({
    ...
    .index("by_venue", ["venueId"])
    .index("by_venue_category", ["venueId", "category"])
    .index("by_venue_name", ["venueId", "name"]),
  ...
  managerGoals: defineTable({
    ...
    .index("by_venue_targetDate", ["venueId", "targetDate"])
    .index("by_venue_status", ["venueId", "status"]),
  venueEvents: defineTable({
    ...
    .index("by_venue_startsAt", ["venueId", "startsAt"])
  ...
  waitlist: defineTable({
    ...
    .index("by_venue_time", ["venueId", "requestedAt"])
    .index("by_venue_status", ["venueId", "status"]),
  tableAssignments: defineTable({
    ...
    .index("by_venue_time", ["venueId", "startsAt"]),
  ...
  tables: defineTable({
    ...
    seatLabelStyle: v.optional(
      v.union(v.literal("number"), v.literal("letter"), v.literal("none")),
    ),
    x: v.number(),
    y: v.number(),
    width: v.number(),
    ...
    .index("by_floor_plan", ["floorPlanId"])
    .index("by_floor_plan_section", ["floorPlanId", "section"]),
  tableStates: defineTable({
    ...
    .index("by_venue", ["venueId"])
    .index("by_venue_merge_group", ["venueId", "mergeGroupId"]),
  tableStateHistory: defineTable({
    ...
    .index("by_table_time", ["tableId", "timestamp"])
    .index("by_venue_time", ["venueId", "timestamp"]),
  ...
  subscriptionEvents: defineTable({
    ...
    .index("by_source_external_id", ["source", "externalEventId"])
    .index("by_venue_time", ["venueId", "processedAt"]),
  webhookRateLimits: defineTable({
    key: v.string(),
