...
const MAX_STRIPE_BYTES = 512_000;
const WEBHOOK_WINDOW_MS = 60_000;
const DEFAULT_WEBHOOK_LIMIT = 60;
...
function hasAcceptablePayloadSize(req: Request, maxBytes: number) {
...
function requestKey(req: Request, route: string) {
  const forwarded = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim();
  const realIp = req.headers.get("x-real-ip")?.trim();
  const auth = req.headers.get("authorization")?.slice(0, 24);
  return `${route}:${forwarded || realIp || auth || "unknown"}`;
}

async function checkWebhookRate(
  ctx: any,
  req: Request,
  route: string,
  limit = DEFAULT_WEBHOOK_LIMIT,
) {
  const result = await ctx.runMutation(internal.webhookRateLimit.check, {
    key: requestKey(req, route),
    limit,
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_STRIPE_BYTES))
      return new Response("Payload too large", { status: 413 });
    const rateLimited = await checkWebhookRate(ctx, req, "stripe", 120);
...
const LEAD_CORS_HEADERS = {
  ...
  "Access-Control-Allow-Headers":
    "authorization, content-type, x-venueflow-leads-secret, x-venueflow-connection-secret",
};

function leadCorsHeaders(req?: Request) {
  const allowed = (process.env.LEADS_ALLOWED_ORIGINS ?? "")
    .split(",")
  ...
  const allowOrigin =
    allowed.length === 0
      ...
      : origin && allowed.includes(origin)
        ? origin
        : allowed[0];
  return { ...LEAD_CORS_HEADERS, "Access-Control-Allow-Origin": allowOrigin };
}

function leadJson(body: unknown, status = 200, req?: Request) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { ...leadCorsHeaders(req), "Content-Type": "application/json" },
  });
}
...
function normaliseLead(value: any, defaultSource?: string) {
  ...
  const fullName =
    cleanString(value.fullName) ??
    cleanString(value.name) ??
    cleanString(value.guestName) ??
    cleanString(value.customerName);
  if (!fullName) return null;
  const email =
    cleanString(value.email) ?? emailFromText(cleanString(value.contact));
  const phone =
    cleanString(value.phone) ??
    cleanString(value.mobile) ??
    phoneFromText(cleanString(value.contact));
  const lead = {
    fullName,
    phone,
    email,
    ...
    marketingOptIn:
      typeof value.marketingOptIn === "boolean"
        ...
        : Boolean(value.optIn ?? value.marketing_opt_in ?? false),
  };
  return Object.fromEntries(
    Object.entries(lead).filter(([, leadValue]) => leadValue !== undefined),
  ) as typeof lead;
}
...
function parseCsvLeads(value: string, defaultSource?: string) {
  return value
    ...
    .filter(
      (line, index) =>
        !(index === 0 && /name/i.test(splitCsvLine(line)[0] ?? "")),
    )
    .map((line) => {
      const [fullName, email, phone, source, company, tags, notes] =
        splitCsvLine(line);
    ...
    .filter((lead): lead is NonNullable<ReturnType<typeof normaliseLead>> =>
      Boolean(lead),
    );
}
  ...
  return normaliseLead(
    {
      fullName,
      email:
        cleanString(value.email) ?? emailFromText(from) ?? emailFromText(text),
...
function leadsFromBody(body: any) {
  ...
  const leads: Array<ReturnType<typeof normaliseLead>> = Array.isArray(
    body?.leads,
  )
    ...
    : body?.lead
      ...
      : [];
  if (cleanString(body?.csv) || cleanString(body?.csvText))
    leads.push(
      ...parseCsvLeads(
        cleanString(body.csv) ?? cleanString(body.csvText) ?? "",
        defaultSource,
      ),
    );
...
http.route({
  ...
  handler: httpAction(
    async (_ctx, req) =>
      new Response(null, { status: 204, headers: leadCorsHeaders(req) }),
  ),
});
...
http.route({
  ...
  method: "POST",
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_JSON_BYTES))
      return leadJson({ error: "Payload too large" }, 413, req);
    const rateLimited = await checkWebhookRate(ctx, req, "crm-leads", 30);
    if (rateLimited) {
    ...
    if (
      !secretOk(
        ...
        receivedSecret(req, "x-venueflow-leads-secret"),
      )
    ) {
      return leadJson({ error: "Unauthorized" }, 401, req);
    }
    let body: any;
    try {
      body = await req.json();
    } catch {
      return leadJson({ error: "Invalid JSON" }, 400, req);
    }
    if (!body?.venueId || typeof body.venueId !== "string")
      return leadJson({ error: "venueId is required" }, 400, req);
    ...
    if (!connectionSecret)
      return leadJson({ error: "Missing connection secret" }, 401, req);
    const leads = leadsFromBody(body);
    if (leads.length === 0)
      return leadJson({ error: "No valid leads found" }, 400, req);
    try {
      const result = await ctx.runMutation(
        internal.guests.ingestLeadsFromWebhook,
        { venueId: body.venueId, leads, connectionSecret },
      );
      return leadJson({ ok: true, ...result }, 200, req);
    } catch (e) {
      console.error("[crm/leads] rejected:", e);
      return leadJson({ error: "Lead ingest rejected" }, 400, req);
    }
  }),
});
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_JSON_BYTES))
      return new Response("Payload too large", { status: 413 });
    const rateLimited = await checkWebhookRate(ctx, req, "pos-checks", 120);
    if (rateLimited) return rateLimited;
    if (
      !secretOk(
        process.env.POS_WEBHOOK_SECRET,
        req.headers.get("x-venueflow-pos-secret"),
      )
    ) {
    ...
    const connectionSecret = req.headers.get("x-venueflow-connection-secret");
    if (!body?.venueId || !body?.provider || !body?.check || !connectionSecret)
    ...
    try {
      await ctx.runMutation(internal.pos.ingestPosCheck, {
        ...
        connectionSecret,
        externalLocationId:
          typeof body.externalLocationId === "string"
            ? body.externalLocationId
            : undefined,
      });
    } catch (e) {
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_JSON_BYTES))
      return new Response("Payload too large", { status: 413 });
    const rateLimited = await checkWebhookRate(ctx, req, "pos-labor", 120);
    if (rateLimited) return rateLimited;
    if (
      !secretOk(
        process.env.POS_WEBHOOK_SECRET,
    ...
    if (body.punches.length > 500)
      return new Response("Too many punches", { status: 400 });
    try {
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_JSON_BYTES))
      return new Response("Payload too large", { status: 413 });
    const rateLimited = await checkWebhookRate(ctx, req, "reservations", 120);
    if (rateLimited) return rateLimited;
    if (
      !secretOk(
        process.env.RESERVATION_WEBHOOK_SECRET,
        req.headers.get("x-venueflow-reservation-secret"),
      )
    ) {
      return new Response("Unauthorized", { status: 401 });
    }
    ...
    try {
      await ctx.runMutation(
        internal.reservationIntegrations.ingestExternalReservation,
        {
          venueId: body.venueId,
          provider: body.provider,
          ...
          connectionSecret,
          externalVenueId:
            typeof body.externalVenueId === "string"
              ? body.externalVenueId
              : undefined,
        },
      );
    } catch (e) {
...
const REVENUECAT_ACTIVE_TYPES = new Set([
  "INITIAL_PURCHASE",
  "RENEWAL",
  "PRODUCT_CHANGE",
  "UNCANCELLATION",
...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    if (!hasAcceptablePayloadSize(req, MAX_JSON_BYTES))
      return new Response("Payload too large", { status: 413 });
    const rateLimited = await checkWebhookRate(ctx, req, "revenuecat", 120);
    if (rateLimited) return rateLimited;
    const expected = process.env.REVENUECAT_WEBHOOK_AUTH;
    const received = req.headers.get("authorization");
    if (!secretOk(expected, received))
      return new Response("Unauthorized", { status: 401 });
    let body: any;
    try {
      body = await req.json();
    ...
    const eventId: string | undefined = event?.id ?? body?.id;
    const type: string | undefined = event?.type;
    if (!appUserId || !eventId || !type)
      return new Response("ok", { status: 200 });
    const expectedEnvironment = process.env.REVENUECAT_ENVIRONMENT;
    if (
      expectedEnvironment &&
      event?.environment &&
    ...
    if (expectedEntitlement && !entitlementIds.includes(expectedEntitlement)) {
      return new Response("ok", { status: 200 });
    }
