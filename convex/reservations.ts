...
const privateEventStatusValue = v.union(v.literal('inquiry'), v.literal('proposal'), v.literal('contract_sent'), v.literal('booked'), v.literal('completed'), v.literal('cancelled'));
...
const reservationValue = v.object({
  ...
  guestCompany: v.union(v.string(), v.null()),
  partySize: v.number(),
  ...
  eventStatus: v.union(privateEventStatusValue, v.null()),
  ...
  contractStatus: v.union(v.string(), v.null()),
  beoStatus: v.union(v.string(), v.null()),
...
function toReservationValue(reservation: Doc<'reservations'>) {
  return {
    ...
    guestCompany: reservation.guestCompany ?? null,
    ...
    contractStatus: reservation.contractStatus ?? null,
    beoStatus: reservation.beoStatus ?? null,
...
async function upsertReservationGuest(ctx: any, args: { venueId: Id<'venues'>; guestName: string; guestPhone?: string; guestEmail?: string; guestCompany?: string; tags: string[]; notes?: string }) {
  const phone = normalizeText(args.guestPhone);
  const email = normalizeText(args.guestEmail)?.toLowerCase();
  const company = normalizeText(args.guestCompany);
...
export const saveReservation = mutation({
  args: {
    ...
    eventStatus: v.optional(privateEventStatusValue),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.reservationId) {
      ...
      const guestId = await upsertReservationGuest(ctx, {
        ...
        guestCompany: args.guestCompany,
        tags: args.tags ?? [],
      ...
      await ctx.db.patch(existing._id, {
        ...
        guestPhone: args.guestPhone,
        guestEmail: args.guestEmail,
        guestCompany: normalizeText(args.guestCompany),
        ...
        occasion: normalizeText(args.occasion),
        notes: args.notes,
        tags: mergeTags(args.tags ?? [], args.isPrivateEvent ? ['private_event'] : []),
        isPrivateEvent: args.isPrivateEvent ?? false,
        eventName: normalizeText(args.eventName),
        eventStatus: args.eventStatus,
        eventSpace: normalizeText(args.eventSpace),
        setupStyle: normalizeText(args.setupStyle),
        menuNotes: normalizeText(args.menuNotes),
        beverageNotes: normalizeText(args.beverageNotes),
        billingNotes: normalizeText(args.billingNotes),
        contractStatus: normalizeText(args.contractStatus),
        beoStatus: normalizeText(args.beoStatus),
        estimatedValueCents: args.estimatedValueCents,
        depositDueCents: args.depositDueCents,
        updatedAt: now,
      });
...
export const exportReservationsCsv = query({
  ...
  handler: async (ctx, args) => {
    ...
    const rows = [['guestName', 'company', 'partySize', 'reservationTime', 'durationMinutes', 'source', 'status', 'phone', 'email', 'occasion', 'tags', 'notes']];
    for (const reservation of reservations.filter((item: Doc<'reservations'>) => !item.deletedAt)) {
      rows.push([
        reservation.guestName,
        reservation.guestCompany ?? '',
