...
const mutationRoutes: Record<string, Route> = {
  ...
  'app.upsertVenueStaff': {
    path: '/v1/app/staff',
    method: 'POST',
    body: ({ venueId, email, fullName, role, jobTitle }) => ({ venueId, email, fullName, role, jobTitle }),
  ...
  'chat.sendMessage': { path: (args) => `/v1/chat/conversations/${args.conversationId}/messages`, method: 'POST', body: ({ text }) => ({ text }) },
